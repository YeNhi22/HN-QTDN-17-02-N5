...
# Cải tiến từ phiên bản cũ: Sửa bug default = fields.Datetime.now() (gọi hàm lúc
# load class Python) → fields.Datetime.now (Odoo gọi lúc tạo record).
# Bug cũ gây ra: mọi bản ghi đều có ngày khấu hao giống nhau (lúc khởi động server).
...
class LichSuKhauHao(models.Model):
    ...
    # Cải tiến từ phiên bản cũ: Sửa bug default = fields.Datetime.now() → fields.Datetime.now
    ngay_khau_hao = fields.Datetime(
        'Ngày khấu hao',
        default=fields.Datetime.now,   # ← ĐÃ SỬA BUG: bỏ ()
        required=True
    )

    gia_tri_hien_tai = fields.Float(
        string='Giá trị ban đầu',
        related='ma_ts.gia_tri_hien_tai',
        store=True
    )
